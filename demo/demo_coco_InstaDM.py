...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("/home/gkinoshita/workspace/Insta-DM/kitti_256/image"),
    )
    parser.add_argument(
        "--output-root-dir",
        type=Path,
        default=Path("/home/gkinoshita/workspace/Insta-DM/kitti_256/segmentation_OneFormer_modified"),
    )
...
if __name__ == "__main__":
    ...
    for image_dir in tqdm.tqdm(args.image_root_dir.iterdir(), disable=not args.output_root_dir):
        for path in tqdm.tqdm(image_dir.glob("*.jpg")):
            ...
            output_dir: Path = args.output_root_dir / image_dir.name
            output_dir.mkdir(parents=True, exist_ok=True)
            output_path = output_dir / path.stem
            instances: Instances = predictions["instances"]
            masks: torch.Tensor = instances.get("pred_masks").cpu().numpy()
            labels: torch.Tensor = instances.get("pred_classes").cpu().numpy()
            np.savez(output_path, masks=masks, labels=labels)
